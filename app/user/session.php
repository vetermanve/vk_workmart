...
function user_session_get_secret($userId) {
    ...
    if (!$secret) {
        ...
        $secret = storage_db_get_value(USER_SESSION_DB_TABLE, 'secret', [
            ['user_id', $userId],
        ]);
    
        core_log('secret found in db: '.$secret);
        
        storage_nosql_set_prefix(CORE_CONFIG_REDIS_MAIN, USER_SESSION_REDIS_KEY_PREFIX, $userId, 
            $secret 
                ? $secret 
...
function user_session_set_secret($userId, $secret) {
    ...
    $dbResult = storage_db_insert_row(USER_SESSION_DB_TABLE, [
        'user_id' => $userId,
        'secret' => $secret,
    ]);
